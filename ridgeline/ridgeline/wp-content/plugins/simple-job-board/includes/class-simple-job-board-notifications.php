...
class Simple_Job_Board_Notifications {
    ...
    public static function job_notification_templates($post_id, $notification_receiver) {
        ...
        $header_title = ( 'applicant' != $notification_receiver ) ? esc_html__( 'Job Application' ) : esc_html__( 'Job Application Acknowledgement' );
        ...
        if ('HR' === $notification_receiver) {
            ...
            $message .= ' has applied against your job opening <b>' . esc_attr( $job_title ) . '</b> at <a href="' . esc_url( $site_url ) . '">' . get_bloginfo('name') . '</a>'
                    . '. Please login to your account to download the CV or check from the applicant\'s list from the dashboard.' . '</p>';
            ...
            $message .= '<p>I am applying for the job post <b>' . esc_attr( $job_title ) . '</b> with interest. I have attached my resume with the job application. I have also filled out the required details.</p>';
            ...
            $message .= '<p>Your application for the position of <b>' . esc_attr( $job_title ) . '</b> at <a href="' . esc_url( $site_url ) . '">' . get_bloginfo('name') . '</a> has been successfully submitted. You will hear back from <a href="' . esc_url( $site_url ) . '">' . get_bloginfo('name') . '</a> based on their evaluation of your CV.</p>'
                    . '<p>Good Luck! </p>'
                    . 'Best Regards,<br>'
                    . 'Admin';
        }
